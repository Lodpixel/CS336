...
def run_linear(
    ...
    """
    module_linear = Linear(d_in, d_out)
    module_linear.load_state_dict({"weight": weights})
    out_features = module_linear.forward(in_features)
    return out_features
...
def run_embedding(
    ...
    """
    module_emb = Embedding(vocab_size, d_model)
    module_emb.load_state_dict({"weight": weights})
    out_emb = module_emb.forward(token_ids)
    return out_emb
...
def run_swiglu(
    ...
    out_swiglu = swiglu_module.forward(in_features)
    return out_swiglu
...
def run_multihead_self_attention(
    ...
    """
    mh_module = multihead_self_attention(d_model, num_heads)
    mh_module.load_state_dict({
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    mh_module = multihead_self_attention(d_model, num_heads, max_seq_len, theta)
    mh_module.load_state_dict({
        "q_proj.weight": q_proj_weight,
        "k_proj.weight": k_proj_weight,
        "v_proj.weight": v_proj_weight,
...
def run_rope(
    ...
    RoPE_module = RoPE_module.to(in_query_or_key.device)
...
def run_transformer_lm(
    ...
    """
    trans_lm_module = transformer_lm(
        vocab_size,
        context_length,
        d_model,
        num_layers,
    ...
    trans_lm_module.load_state_dict(weights)
    out_indices = trans_lm_module.forward(in_indices)
    return out_indices
...
def run_rmsnorm(
    ...
    out_rmsnorm = rmsnorm_module.forward(in_features)
    return out_rmsnorm
